...
export const NetworkPage: React.FC<NetworkPageProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'cells' | 'hierarchy'>('cells');
    ...
    <div className="space-y-4 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 sm:p-4 space-y-3">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span>Sector Cells Grid ({cells.length})</span>
                </button>
                ...
                >
                  ...
                  <span>Site & Cluster Hierarchy ({INITIAL_SITES.length} Sites)</span>
                </button>
              </div>

              <span className="text-[11px] text-slate-500 font-mono-num hidden sm:inline">
                Wilaya ➔ Site ➔ Cell Sector
              </span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search Cell ID or Site (e.g. CELL-003, Bab Ezzouar, Mansourah)..."
                  value={searchCell}
          ...
          {/* VIEW MODE 2: Physical Site & Cluster Hierarchy */}
          {viewMode === 'hierarchy' && (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2">
                  <Workflow className="w-4 h-4 text-sky-400" />
                  <span>Physical Base Station Sites & Radio Sectors</span>
                </h3>
                <span className="text-[11px] text-slate-500 font-mono-num">
                  3GPP Logical Topology
                </span>
              </div>

              <div className="space-y-3">
                {INITIAL_SITES.map(site => {
                  const isDegraded = site.status === 'anomaly';
                  const isWarning = site.status === 'warning';

                  return (
                    <div
                      key={site.siteId}
                      className="bg-slate-950 border border-slate-800 rounded-lg p-3.5 space-y-2.5 hover:border-slate-700 transition-all"
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/70 pb-2">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-xs font-bold text-sky-400">{site.siteId}</span>
                            <span className="text-white font-bold text-xs">{site.siteName}</span>
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-800 text-slate-300 font-mono-num">
                              {site.wilaya}
                            </span>
                          </div>
                          <span className="text-[11px] text-slate-400 mt-0.5 block">
                            Type: <strong className="text-slate-300">{site.siteType}</strong> • Band: {site.carrierBand}
                          </span>
                        </div>

                        <div className="flex items-center gap-3 text-xs font-mono-num">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                            isDegraded ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' :
                            isWarning ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' :
                            'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                          }`}>
                            Health: {site.healthScore}%
                          </span>
                          <span className="text-slate-400">RTT: {site.backhaulLatencyMs}ms</span>
                        </div>
                      </div>

                      {/* Child Sector Cells */}
                      <div className="flex items-center gap-2 pt-1 flex-wrap">
                        <span className="text-[11px] text-slate-500 uppercase font-semibold">Sectors:</span>
                        {site.cells.map(cellIdentifier => {
                          const matchedCell = cells.find(c => c.cellId === cellIdentifier);
                          const isCurSelected = currentCell.cellId === cellIdentifier;
                          const cellStatus = matchedCell?.status || 'normal';
                          ...
                          return (
                            ...
                            >
                              ...
                              <span>{cellIdentifier}</span>
                              {matchedCell && (
                                <span className="text-[10px] opacity-80">
                                  ({matchedCell.users} users)
                                </span>
                              )}
                            </button>
                          );
